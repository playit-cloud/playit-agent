...
pub(crate) fn configure_socket_permissions(socket_path: &str) -> Result<(), IpcError> {
    let Some(target) = socket_permission_target(
        socket_path,
        playit_service_manager::unix_account::effective_uid(),
    ) else {
        return Ok(());
    };
    ...
    let group_gid = if target.chown_group {
        match playit_service_manager::unix_account::group_gid_by_name(target.group_name) {
