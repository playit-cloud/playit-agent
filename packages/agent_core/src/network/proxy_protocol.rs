...
impl ProxyProtocolHeader {
    pub fn from_udp_flow(flow: &UdpFlow) -> Self {
        match flow {
            UdpFlow::V4 { src, dst, .. } => ProxyProtocolHeader::AfInet {
                client_ip: *src.ip(),
                proxy_ip: *dst.ip(),
                client_port: src.port(),
                proxy_port: dst.port(),
            },
            UdpFlow::V6 { src, dst, .. } => ProxyProtocolHeader::AfInet6 {
                client_ip: src.0,
                proxy_ip: dst.0,
                client_port: src.1,
                proxy_port: dst.1,
...
impl ProxyProtocolHeader {
    pub async fn write_v1_tcp<W: AsyncWrite + Unpin>(
        &self,
        out: &mut W,
    ) -> Result<(), std::io::Error> {
    ...
    pub async fn write_v2_tcp<W: AsyncWrite + Unpin>(
        &self,
        out: &mut W,
    ) -> Result<(), std::io::Error> {
