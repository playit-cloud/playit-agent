...
RUN mkdir -p packages/playit-cli/src && mkdir -p packages/playit-ipc/src && mkdir -p packages/playitd/src && mkdir -p packages/playitd-tray/src && mkdir -p packages/agent_core/src && mkdir -p packages/agent_proto/src && mkdir -p packages/api_client/src
...
RUN touch packages/playit-cli/src/lib.rs && touch packages/playit-ipc/src/lib.rs && touch packages/playitd/src/lib.rs && touch packages/playitd-tray/src/main.rs && touch packages/agent_core/src/lib.rs && touch packages/agent_proto/src/lib.rs && touch packages/api_client/src/lib.rs
RUN cargo fetch
